...
u_char * KHttp2::state_process_header(u_char *pos,	u_char *end)
{
	...
	r->parse_header(header->name.data,header->name.len, header->value.data,header->value.len,false);
	return state_header_complete(pos, end);
}
...
bool KHttp2::ReadHeaderSuccess(KHttp2Context *stream)
{
	...
	if (!KBIT_TEST(rq->req.flags, RQ_HAS_CONTENT_LEN) && !stream->in_closed) {
		rq->req.content_length = -1;
	}
